  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (cmd === "create") {
      ...
      try {
        ...
        // Default = 1 (solo). If party > 1 the player MUST complete the party before start,
        // otherwise inviteStatus becomes PartyNotFull and no match is ever generated for them.
        const party = interaction.options.getInteger("party") || 1;
        ...
        // Signup opens immediately so the tournament shows as joinable in-game right away
        const signupStart = new Date(Date.now() - 5 * 1000);
        ...
        await CreateTournament({
          ...
          PartySize: party,
          Status: 1, // InvitationOpen — visible & joinable in the game client
          TournamentType: 1,
          Phases: phases,
          Region: region,
          RoundCount: rounds,
          CurrentPhaseId: 0,
          // When PartySize=1 → 1v1 style (min 1 team can start waiting, max 2 teams per match)
          // When PartySize>1 → always 2 full parties per match
          MinPlayersPerMatch: party === 1 ? 1 : 2,
    ...
    if (cmd === "delete") {
      ...
      try {
        ...
        // Full cleanup: remove matches, user signups, then the tournament itself
        // (same logic as TournamentCleaner for finished tournaments)
        ...
        const TournamentId = id.toString();
        ...
        const e = new EmbedBuilder()
          .setTitle("deleted")
          .setDescription(`deleted \`${id}\`\n(matches and player signups also removed)`)
          .setColor("#ff4444")
          .setTimestamp();
